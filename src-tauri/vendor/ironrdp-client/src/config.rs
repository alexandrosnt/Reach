...
pub struct Config {
    ...
    /// Reach: makes an H.264 decoder for each connection's graphics pipeline.
    pub(crate) h264_decoder_factory: Option<std::sync::Arc<dyn Fn() -> Option<Box<dyn ironrdp_egfx::decode::H264Decoder>> + Send + Sync>>,
...
impl ConfigBuilder {
    ...
    pub fn with_h264_decoder_factory(
        mut self,
        factory: std::sync::Arc<dyn Fn() -> Option<Box<dyn ironrdp_egfx::decode::H264Decoder>> + Send + Sync>,
    ) -> Self {
        self.h264_decoder_factory = Some(factory);
        self
    }
    ...
    pub fn build(mut self) -> anyhow::Result<Config> {
        ...
        Ok(Config {
            ...
            h264_decoder_factory: self.h264_decoder_factory,
