...
- **Remote Desktop (RDP)** · A Windows desktop in a tab, on every platform Reach runs on. Clipboard and files go both ways, sound plays, a local folder can be shared in as a drive, video comes as H.264, and the server's certificate goes through the same trust-on-first-use prompt as an SSH host key.
