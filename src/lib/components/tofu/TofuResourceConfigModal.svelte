<script lang="ts">
	...
	let { resourceId, onclose }: Props = $props();

	let fields = $state<Record<string, unknown>>({});
	let logicalName = $state('');
	let nameError = $state<string | null>(null);

	let resourceConfig = $derived(getActiveResources().find((r) => r.id === resourceId));
	let catalogEntry = $derived(getResourceCatalog().find((c) => c.id === resourceConfig?.resourceType));

	onMount(() => {
		const config = getActiveResources().find((r) => r.id === resourceId);
		if (config) {
	...
	function validateName(): boolean {
		...
		const duplicate = getActiveResources().find(
			(r) =>
				r.id !== resourceId &&
				r.resourceType === resourceConfig?.resourceType &&
...
<div class="overlay" onclick={handleBackdropClick} onkeydown={() => {}}>
	<div class="modal" role="dialog" aria-modal="true" aria-label={catalogEntry?.name ?? t('tofu.configure_resource')}>
		<header class="modal-header">
			<h2 class="modal-title">{catalogEntry?.name ?? resourceId}</h2>
		...
		<div class="modal-body">
			<div class="field-group">
				<label class="field-label" for="resource-logical-name">{t('tofu.resource_logical_name')}</label>
				<input
					id="resource-logical-name"
					type="text"
					class="field-input"
