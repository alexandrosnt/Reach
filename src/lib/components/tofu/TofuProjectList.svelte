<script lang="ts">
	...
	import { getProjects, loadProjects, openProject, deleteProject, applyTemplate } from '$lib/state/tofu.svelte';
	import Button from '$lib/components/shared/Button.svelte';
	import TofuProjectEditor from './TofuProjectEditor.svelte';
	import TofuTemplatePicker from './TofuTemplatePicker.svelte';
...
<div class="project-list">
	<header class="header">
		<h2 class="title">{t('tofu.projects')}</h2>
		<div class="header-actions">
			<Button variant="secondary" size="sm" onclick={() => (showTemplatePicker = true)}>
				{t('tofu.from_template')}
			</Button>
	...
	{#if projectList.length === 0}
		<div class="empty-state">
			...
			<p class="empty-text">{t('tofu.no_projects')}</p>
		</div>
	{:else}
		<div class="grid">
				...
				>
					...
					{#if project.lastOpenedAt}
						<p class="card-timestamp">
							{t('tofu.last_opened')}: {formatTimestamp(project.lastOpenedAt)}
						</p>
					{/if}
				</div>
			{/each}
	...
	{#if deletingId !== null}
		<div class="confirm-overlay" role="presentation" onclick={cancelDelete}>
			...
			<div class="confirm-dialog" role="alertdialog" tabindex="-1" onclick={(e) => e.stopPropagation()}>
				<p class="confirm-text">{t('tofu.delete_confirm')}</p>
	...
	<TofuProjectEditor
		...
		onclose={() => { showEditor = false; pendingTemplateId = null; }}
		oncreate={async (project) => {
			showEditor = false;
			if (pendingTemplateId) {
				try {
					await applyTemplate(project.id, pendingTemplateId);
				} catch { /* template apply failed, project still created */ }
